...
int main(int argc, char** argv) {
    ...
    std::cout << "Reading files from directory: " << img_dir << std::endl;
    ...
    for (int index = 0; index < file_names.size(); ++index) {
        // std::cout << "Image file path: " << file_names[index] << std::endl;
        auto img = cv::imread(file_names[index]);
        // std::cout << "Image step: " << img.step << ", Image data: " << static_cast<void*>(img.data) << std::endl;
        ...
        for (int b = 0; b < BATCH_SIZE; b++) {
            ...
            for (int row = 0; row < INPUT_H; ++row) {
                if (img.empty()) {
                    std::cerr << "Error: Could not open or find the image: " << file_names[index] << std::endl;
                    continue;
                }
        ...
        for (int b = 0; b < BATCH_SIZE; b++) {
            ...
            /// std::string img_dir = "/home/ubuntu-test/tensorrtx/real-esrgan/general-x4v3/"; // Đường dẫn tới thư mục
            /// cv::imwrite(img_dir + "_" + file_names[index] + ".jpg", img_res);
            /// std::cout << "Writing files to directory: " << img_dir << std::endl;
            std::string output_dir = "/home/ubuntu-test/tensorrtx/real-esrgan/general-x4v3/";
            std::string input_filename = getFileName(file_names[index]);
            std::string output_path = output_dir + "output_" + input_filename;
